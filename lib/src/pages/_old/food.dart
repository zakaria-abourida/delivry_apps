...
class _FoodWidgetState extends StateMVC<FoodWidget> with TickerProviderStateMixin {
  FoodController _con;
  ...
  Widget build(BuildContext context) {
    ...
    : ScopedModelDescendant<RefreshModel>(
          ...
          return Scaffold(
            ...
            appBar: AppBar(
              ...
              leading: IconButton(
                  ...
                  onPressed: () {
                    if (!addedToCart) Navigator.pop(context);
                  }),
                                      ...
                                      children: <Widget>[
                                        ...
                                        ListView.separated(
                                          ...
                                          itemBuilder: (context, extraIndex) {
                                            ...
                                            var extra  = _con.food.extras.where((extra) => extra.extraGroupId == extraGroup.id)
                                                .elementAt(extraIndex);

                                            var extras  = _con.food.extras.where((extra) => extra.extraGroupId == extraGroup.id);
                                            ...
                                            if(freeExtraVerification()){   
                                              if (currentUser.value.apiToken ==
                                                ...
                                                if (_con.isSameRestaurants(
                                                  ...
                                                  //.whenComplete(() =>  new Future.delayed(const Duration(milliseconds: 3000),()=>Navigator.pop(context)));

                                                  // .pushReplacementNamed(
                                                  //   '/Menu',
                                                  //   arguments: RouteArgument(id: _con.food.id, heroTag: widget.routeArgument.heroTag))
                                                  // );
                                                }
                                              }
                                            }else{
                                              vdialog.pop(context,"Vous devez selectionner un supplément");
                                            }
                                          },
                                          padding: EdgeInsets.symmetric(
                                              vertical: 14),
      ...
      for (final group in _con.food.extraGroups) {
        ...
        for (final extra in _con.food.extras) {
          if (extra.extraGroupId == group.id) {
            if (extra.price == 0 && !extra.checked) {
              condition_satisfied = false;
            } else if (extra.price == 0 && extra.checked) {
              condition_satisfied = true;
              break;
            }
          }
