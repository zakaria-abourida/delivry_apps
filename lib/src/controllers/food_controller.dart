...
class FoodController extends ControllerMVC {
  Food food;
  CartController _con2 = new CartController();
  double quantity = 1;
  double total = 0;
  List<Cart> carts = [];
  Favorite favorite;
  ...
  void listenForFood({String foodId, String message}) async {
    ...
    stream.listen((Food _food) {
      ...
      print(a);
      vdialog.pop(context, S.of(context).verify_your_internet_connection);
  ...
  Future<void> addToCart(Food food, {bool reset = false}) async {
    try {
      setState(() {
        this.loadCart = true;
      });
      ...
      if (_oldCart != null) {
        ...
        updateCart(_oldCart).then((value) {
          ...
          // print(S.of(context).this_food_was_added_to_cart);
  ...
  void addToFavorite(Food food) async {
    ...
    addFavorite(_favorite).then((value) {
      ...
      vdialog.pop(context, S.of(context).thisFoodWasAddedToFavorite);
  ...
  void removeFromFavorite(Favorite _favorite) async {
    removeFavorite(_favorite).then((value) {
      ...
      vdialog.pop(context, S.of(context).thisFoodWasRemovedFromFavorites);
