...
//    _chatRepository.signInWithToken(currentUser.value.apiToken);
  ...
  listenForChats(Conversation _conversation) async {
    _chatRepository.getUserMessages();
    final Stream<Chat> stream = await _chatRepository.getUserMessages();
    stream.listen((Chat _chat) {
      setState(() => chats.add(_chat));
    }, onError: (a) {
      print(a);
    }, onDone: () {});
  }
  ...
  addMessage(Conversation _conversation, String text) {
    Chat _chat = new Chat(text, DateTime.now().toUtc().millisecondsSinceEpoch, "1", currentUser.value.id);
  ...
  sendMessage(Conversation _conversation, String text) {

     newMessage(text, S.of(context).newMessageFrom + " " + currentUser.value.name);
