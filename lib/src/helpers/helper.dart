...
//        },
  ...
  static Future<Marker> getMyPositionMarker(double latitude, double longitude,
      {path, MarkerId markerId}) async {
    final Uint8List markerIcon =
        await getBytesFromAsset(path ?? 'assets/img/my_marker.png', 120);
    Marker marker = Marker(
        markerId: markerId ??
            MarkerId(Random(DateTime.now().millisecondsSinceEpoch).toString()),
        icon: BitmapDescriptor.fromBytes(markerIcon),
        anchor: Offset(0.5, 0.5),
  ...
  static double getTotalOrdersPrice(Order order) {
    ...
    if (order.discountAmount != null) total -= order.discountAmount;
    return total;
  }
  ...
  static String convertDistanceToKm(int distance) {
  ...
  static double simpleConvertDistanceToKm(int distance) {
    double distanceInKm = distance / 1000;
    return distanceInKm;
  }

  static double calculateCourierFeesByDistance(int distance) {
    double distanceInKm = distance / 1000;
    double priceByKm = double.parse(setting.value.priceByKm);
    double totalFees = distanceInKm * priceByKm;
    return totalFees.ceilToDouble();
  }
  ...
  static CameraPosition getCPosition(double lat, double long) {
  ...
  static phoneLaunchUri(String phone) {
    return UrlLauncher.launch('tel:$phone');
  }

  static List<String> courierAvailabilityTimes() {
    List<String> list = ['Dès que possible'];
    if (setting.value.coursierEndTime == null) return list;
    int endTime = int.parse(setting.value.coursierEndTime.split(':')[0]);
    var currentDate = DateTime.now();
    for (var i = currentDate.hour + 1; i < endTime; i++) {
      var item = '${i}h00 - ${i + 1}h00';
      list.add(item);
    }
    return list;
  }
  ...
  static String splitName(String name) {
    ...
    splitName.forEach((element) {
      if (result.length < 2) result += element.substring(0, 1);
    });
